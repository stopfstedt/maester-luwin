...
class Messenger {
  constructor(client, msg, cardList) {
    this.promises = [];
    this.client = client;
    this.msg = msg;
    this.pattern = /\[\[([^\]]+)\]\]/g
    ...
    Promise.all(this.promises).then(embeds => {
      embeds.forEach(embed => {
        this.msg.channel.send(new MessageEmbed(embed));
      });
...
Messenger.prototype.defaultResponseType = ResponseTypes.ImageResponse;
Messenger.prototype.specialResponseTypes = {
  '!': ResponseTypes.TextResponse
};
