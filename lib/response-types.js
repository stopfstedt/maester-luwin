...
const { makeKeyFromSetCode, makeKeyFromName, extractNameAndSetCode } = require(__dirname + "/card-utils.js");
...
class TextResponse {
  ...
  lookupCard() {
    return new Promise((resolve, reject) => {
      try {
        const nameAndSet = extractNameAndSetCode(this.cardName);
        if (false === nameAndSet) {
          ...
          const key = makeKeyFromName(nameAndSet.name);
          const code = makeKeyFromSetCode(nameAndSet.setCode);
